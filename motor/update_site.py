...
def suffix_to_season_meta(suffix: str):
    ...
    if m:
        year = m.group(2)
        return {"id": f"leto-{year}", "label": f"Летња развојна лига {year}", "icon": "☀️"}

    # редовна сезона: "202526", "2025-26", "2025/26", "2025_26"
    m = re.match(r"^(\d{4})\s*[-/_]?\s*(\d{2})$", s)
    if m:
        y1, y2 = m.group(1), m.group(2)
        return {"id": f"{y1}-{y2}", "label": f"Сезона {y1}/{y2}", "icon": "🏆"}
...
def process_season(suffix: str, files: dict):
    ...
    date_from = format_srb_date(dates[0]) if dates else None
    date_to = format_srb_date(dates[-1]) if dates else None
